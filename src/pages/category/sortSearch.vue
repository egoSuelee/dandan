			...
			<div class="fixed-search-header">
				<div class="searchWrapper">
						<div class="searchField">
							...
							<input type="text" name="search_text" class="input-search" v-model="searchText"/>
						</div>
						<div class="search-title">
							<span class="search-title-text">搜索</span>
						</div>
				</div>
				<div class="search-typeWrapper">
					<ul class="type-list">
						<li class="type-item" v-for = "(type, index) in searchTypes" :class = "{'selected-type-item': index == selectedTypeIndex}" @click = "typeItemDidSelectd(index)">
							<span class="type-text">{{type.title}}</span>
						</li>
					</ul>
				</div>
			</div>
			<div class="goodsWrapper">
				<ul v-load-more ="loadMoreGoods">
...
export default{
	...
	created() {
		this.getGoodsWithType(this.selectedTypeIndex, this.rankType, this.number_of_pages);
	},
	mixins: [loadMore],
	methods: {
		...mapMutations([
		...
		getGoodsWithType(sort_type, fage, number_of_pages){
			let params = {"Sort_type": sort_type, "fage": fage, "Number_of_pages": number_of_pages, "cStoreNo": "000"};
			console.log("*****@@@@@@@*******");
			this.$api.post("/Simple_online/Select_Search_Goods", params, result => {
		...
		.fixed-search-header {
			...
			.search-typeWrapper {
				.type-list {
					...
					display: -webkit-flex;
					background-color: white;
					@include border1px(rgba(7, 17, 27, .45));
					.type-item {
						flex: 1;
						text-align: center;
						font-size: .75rem;
